...
(function () {
  async function INS_summarize(text) {
    ...
    try {
      ...
      console.log('[INS_Reader][ai-client] sendMessage 返回:', resp, 'chrome.runtime.lastError:', chrome.runtime.lastError);
    } catch (err) {
      console.error('[INS_Reader][ai-client] sendMessage 抛出异常:', err.name, err.message, err);
      throw new Error('无法连接 AI 服务，请确认后端已启动');
    }

    if (!resp) {
      console.error('[INS_Reader][ai-client] resp 为空（service worker 可能未响应或已休眠），lastError:', chrome.runtime.lastError);
      throw new Error('无法连接 AI 服务，请确认后端已启动');
    }
    if (resp.status === 429) {
