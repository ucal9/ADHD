...
// INS_Reader · AI 客户端模块
// 职责：调用 INS_Reader 后端的 AI 摘要/高亮代理接口（后端持有 LLM Key，前端不接触密钥）。
// 依赖 INS_Reader.prefsStore 读取 deviceId。后端不可用时功能整体降级为不可用，
// 不影响正文定位/降噪/排版等核心本地功能。
// 调用者：panel-ui.js 点击"生成摘要"按钮时调用 summarize()，点击"应用高亮"按钮时调用 highlight()。
//
...
(function () {
  ...
  async function INS_highlight(text, options) {
    const { prefsStore } = window.INS_Reader;
    const prefs = prefsStore.get();

    console.log('[INS_Reader][ai-client] 发起高亮请求，正文长度:', text.length, 'options:', options);

    let resp;
    try {
      resp = await INS_sendWithTimeout({
        type: 'INS_READER_AI_HIGHLIGHT',
        payload: { device_id: prefs.deviceId, text, mode: 'highlight', options },
      });
    } catch (err) {
      console.error('[INS_Reader][ai-client] highlight 请求阶段失败:', {
        name: err && err.name,
        message: err && err.message,
        stack: err && err.stack,
        lastError: chrome.runtime.lastError,
    ...
    console.log('[INS_Reader][ai-client] 高亮处理成功，长度:', resp.result.length);
    return resp.result;
  }

  window.INS_Reader.aiClient = {
    summarize: INS_summarize,
    highlight: INS_highlight,
  };
})();
